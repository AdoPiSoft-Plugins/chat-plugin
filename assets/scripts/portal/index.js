...
function formatDate(date) {
  ...
  var mm_dd_yy = [date.getMonth() + 1, date.getDate(), date.getFullYear()].join("/");
...
function scrollToBottom(el) {
  if (!el) el = document.querySelector(".conversation");
...
function initSettings(cb) {
  httpGet(settings_api_url, function(data) {
    ...
    hide_portal_button = data.config.hide_portal_button;
...
function initChats() {
  ...
  httpGet(device_api_url, function(device_data) {
    ...
    httpGet(chats_api_url, function(data) {
      ...
      var has_unread = chats.findIndex(function(c) {
        return !c.is_read_by_user
      }) >= 0;
  ...
  var i = setInterval(function() {
    ...
    if (document.querySelector(".chat-box") && socket) {
      initSettings(function() {
        ...
        if(icon_filename){
          document.getElementById('chat-icon').src = '/plugins/chat-plugin/assets/images/icon/' + icon_filename
        }
