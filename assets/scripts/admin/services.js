(function() {
  ...
  App.service("ChatService", ["$http", "toastr", "CatchHttpError", "$q", function($http, toastr, CatchHttpError, $q) {
    ...
    this.bulkSendToClients = function(message) {
      return $http.post("/chat-plugin/chats/bulk-send", {
        message: message
    ...
    this.restoreIconSettings = function() {
      return $http.post("/chat-plugin/setting/restore-icon").catch(CatchHttpError)
    }
  }]).service("DevicesService", ["$http", "toastr", "CatchHttpError", "$q", function($http, toastr, CatchHttpError, $q) {
