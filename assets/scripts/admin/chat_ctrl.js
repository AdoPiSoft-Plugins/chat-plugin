  ...
  App.component("chats", {
    ...
    $scope.loadMore = function() {
      ...
      ChatService.getClientMessages($scope.contact.id, opts).then(function(res) {
        ...
        $scope.chats = _.uniq(chats, function(i) {
          return i.id
        });
        $scope.has_more = data.total_count > $scope.chats.length
      })
    ...
    $scope.muteDevice = function(device) {
      return DevicesService.muteDevice(device.id).then(function() {
        ...
        toastr.success(device.hostname || device.mac_address + " successfully muted")
      })
    };
    $scope.unmuteDevice = function(device) {
      return DevicesService.unmuteDevice(device.id).then(function() {
        device.is_muted = false;
        toastr.success(device.hostname || device.mac_address + " successfully unmuted")
      })
    };
    socket.on("chat", function(chat) {
      if ($(".conversation.device-" + chat.mobile_device_id + ":visible").length) {
        var exists = _.findIndex($scope.chats, function(c) {
          return c.id == chat.id
        }) >= 0;
        if (exists) return;
