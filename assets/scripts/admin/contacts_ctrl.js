  ...
  App.component("contacts", {
    ...
    $scope.loadContacts = function(opts) {
      ...
      DevicesService.get(opts).then(function(res) {
        ...
        $scope.$parent.has_unread = _.findIndex($scope.devices, function(d) {
          return d.has_unread
        }) >= 0;
    ...
    socket.on("chat", function(chat) {
      if (chat.sender_id) {
        var dIndex = _.findIndex($scope.devices, function(d) {
          return d.id == chat.sender_id
        });
        if (dIndex >= 0) {
